...
- **TECH-012 (Parser Choice Policy)**: When parsing strings that belong to a language more complex than what a finite-state machine can robustly represent, do not use regex unless the regex is trivial; use a proper language parser.
- **TECH-013 (No Placeholder Artifacts)**: Do not introduce placeholders, stubs, tautologies, or mock implementations without explicit user approval. If explicitly approved, include a clear banner in both chat and file content marking the artifact as temporary.
